...
const Paginator = ({totalCountItems, pageSize, currentPage, onPageClick, portionSize=5}) => {
...
{pages.filter(p => p >= leftPortionPageNumber && p <= rightPortionPageNumber).map(p => {
  return <span className={currentPage === p && u.selectedPage} onClick={(e) => { onPageClick(p) }}>{p}</span>
})}
